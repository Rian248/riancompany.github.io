<?php
function url_dasar(){
    //$_SERVER['SERVER_ALAMAT'] : alamat website, misalkan websitemu.com
    //$_SERVER['SCRIPT_NAME'] : directory website, misalkan websitemu.com/blog/ $_SERVER['SCRIPT_NAME'] : blog
    $url_dasar  = "http://localhost:8080/".dirname($_SERVER['SCRIPT_NAME']);
    return $url_dasar;
}
...
function tutors_foto($id){
    ...
    $q1     = mysqli_query($koneksi,$sql1);
    $r1     = mysqli_fetch_array($q1);
    $foto   = strip_tags($r1['foto']);

    if($foto){
        return $foto;
    }else{
        return 'tutors_default_picture.jpeg';
    }
}
...
function kirim_email($email_penerima, $nama_penerima, $judul_email, $isi_email){
    ...
    $email_pengirim     = "rianmhmmd248@gmail.com";
